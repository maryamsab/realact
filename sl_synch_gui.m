...
% Last Modified by GUIDE v2.5 13-Mar-2015 19:40:23
...
% Set the value of the gain slider, with max/min of +10/-10, convert from max/min of 0/1
% slider_position = max(0,min(1,(str2double(value) + 10)/20));
slider_position = (str2double(value)/10);
set(handles.slider_Gain,'Value',slider_position);

% Assign gui, startstop, and gain handles to the base workspace
...
value = num2str(slider_position*10);
% Update the model's gain value
set_param([bdroot '/Gain'],'Gain',value)
...
% --- Executes during object creation, after setting all properties.
function pushbutton_startstop_CreateFcn(hObject, eventdata, handles)
% hObject    handle to pushbutton_startstop (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
