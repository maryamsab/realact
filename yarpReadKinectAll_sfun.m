function yarpReadKinectAll_sfun(block)
%%
...
%        jointsList ={ 'tag wristright'};  
        item = bottleIn.get(0);                                  
%        if(block.Dwork(1).Data<24)
%            block.Dwork(1).Data=block.Dwork(1).Data+1;
%        else
%            block.Dwork(1).Data=1;
%        end
%        jointID=block.Dwork(1).Data;
        jointID=6;
        jointTag = 'tag shoulderright';
%        jointTag = jointsList(jointID);  
        disp('--------jointFromKinectBottle:------');
        disp(jointID);
        disp(jointTag);
