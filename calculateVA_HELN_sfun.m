function calculateVA_HELN_sfun(block)
%Maryam 30/06/2015
...
block.OutputPort(1).Data = 10;%V=s+/i-f/d-s, A=s+/i-f/d-s
block.OutputPort(2).Data = -10; 
block.Dwork(1).Data = 10;%initial value
...
function Update(block)
    ...
    sTime=round(rto); 
    HighEdecayRate=1; %LowN goes faster to steady-state than others
 
    MAX_V=100;%Valence curve cieling
    MAX_A=100;%Arousal curve cieling
    
    valenceTrigger = block.InputPort(1).Data;%agent satisfaction
    ...
    if valenceTrigger==1   %HELN high reponse to positive stimuli
        if(block.Dwork(1).Data<MAX_V)%emotion changes btw 20,-20
                block.Dwork(1).Data = block.Dwork(1).Data + (B_FACTOR*triggerWeightV); %V=s+/i-f/d-s
         end
    elseif valenceTrigger== -1 %HELN low reponse to negative stimuli
        if(block.Dwork(1).Data>(-1*MAX_V))
                block.Dwork(1).Data = block.Dwork(1).Data - (S_FACTOR*triggerWeightV); 
                %rate = -0.05;
        end
    end  
    if arousalTrigger==1    %HELN low arousal 
        if(block.Dwork(2).Data<MAX_A)%emotion changes btw 20,-20
                block.Dwork(2).Data = block.Dwork(2).Data + (S_FACTOR*triggerWeightA); %A=s+/i-f/d-s
         end
    elseif arousalTrigger== -1  %HELN low arousal 
    ...
    %HELN initial: V+
    block.Dwork(1).Data = 10 -((10 -block.Dwork(1).Data)*exp(decayRate*sTime));
