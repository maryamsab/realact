function calculateVA_LEHN_sfun(block)
...
function InitializeConditions(block)
    ...
    block.OutputPort(1).Data = -20; %V=s-/i-s/d-f, A=s+/i-f/d-s
    block.OutputPort(2).Data = 20; 
...
function Update(block)
    ...
    sTime=round(rto); 
    HighNdecayRate=1; %highN goes  5 times slower to steady-state than others
    ...
    decayRate=HighNdecayRate*DECAY;
    if valenceTrigger==1   %LEHN low reponse to positive stimuli
        if(block.Dwork(1).Data<MAX_V)%emotion changes btw 20,-20
                block.Dwork(1).Data = block.Dwork(1).Data + (S_FACTOR*triggerWeightV); %V=s-/i-s/d-f
        end
    elseif valenceTrigger== -1 %LEHN high reponse to negative stimuli
        if(block.Dwork(1).Data>(-1*MAX_V))
                block.Dwork(1).Data = block.Dwork(1).Data - (B_FACTOR*triggerWeightV); 
        end
    end  
    if arousalTrigger==1   %LEHN highly aroused
    %for introverts this internal emotion is not expressed,check facial expr,gaze and pose module
        if(block.Dwork(2).Data<MAX_A)%emotion changes btw 20,-20
            block.Dwork(2).Data = block.Dwork(2).Data + (B_FACTOR*triggerWeightA); % A=s+/i-f/d-s
        end
    elseif arousalTrigger== -1 %LEHN highly aroused 
        if(block.Dwork(2).Data>(-1*MAX_A))
                block.Dwork(2).Data = block.Dwork(2).Data - (S_FACTOR*triggerWeightA); 
        end
    end   
    %initial for LEHN: V-A+
    %tendency for returning to initial is  X=Xss - (Xss - X0)e-t/K
    %based on http://www.hpmuseum.org/software/33/33expgd.htm     
    block.Dwork(1).Data = -20 -((-20 -block.Dwork(1).Data)*exp(decayRate*sTime)); 
